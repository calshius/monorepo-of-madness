...
class RecipeProvider {
    constructor() {
        this.cache = new Map();
        this.baseUrl = 'https://www.bbc.co.uk/food/recipes';
    }
    async prepareInvocation(options, _token) {
        const confirmationMessages = {
            title: 'Search BBC Food for Recipes',
            message: new vscode.MarkdownString(`Search BBC Food for "${options.input.query}" recipes?` +
        ...
        try {
            ...
            if (recipes.length === 0) {
                return new vscode.LanguageModelToolResult([
                    new vscode.LanguageModelTextPart(`No recipes found for "${input.query}". Try searching for ingredients like "chicken", "pasta", or dish names like "curry", "soup".`)
                ]);
            }
            return new vscode.LanguageModelToolResult([
                new vscode.LanguageModelTextPart(this.formatRecipes(recipes, input.query))
            ]);
        }
        catch (error) {
            return new vscode.LanguageModelToolResult([
                new vscode.LanguageModelTextPart(`Error searching for recipes: ${error instanceof Error ? error.message : 'Unknown error'}`)
            ]);
        }
    }
    async searchRecipes(query, maxResults) {
        // Check cache first
        ...
        $('a[href*="/food/recipes/"]').each((index, element) => {
            if (index >= maxResults)
                return false; // Stop when we have enough
            const $link = $(element);
            ...
            if (!href || !linkText || linkText.length < 10)
                return; // Skip if not enough info
